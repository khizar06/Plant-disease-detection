...
if not st.session_state.logged_in:
    ...
    st.markdown(
        "<h1 style='text-align:center;color:#1b5e20;'>🌿 Plant AI System</h1>",
        unsafe_allow_html=True
    )

    st.markdown(
        "<h4 style='text-align:center;color:gray;'>AI Based Plant Disease Detection</h4>",
        unsafe_allow_html=True
    )
    ...
    # =====================================
        ...
        if st.button("Recover Password"):
            ...
            if (
                username in users and
                users[username]["recovery"] == recovery
            ):

                st.success(
                    f"Your Password: {users[username]['password']}"
                )

            else:

                st.error("Wrong Username or Recovery Code")
    ...
    # =====================================
        ...
        if st.button("Update Password"):
            ...
            if (
                username in users and
                users[username]["password"] == old_password
            ):
